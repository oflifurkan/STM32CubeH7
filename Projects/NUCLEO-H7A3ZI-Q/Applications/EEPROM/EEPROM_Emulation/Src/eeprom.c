...
uint16_t EE_Init(void)
{
  ...
  uint32_t Page0_Start_Address = PAGE0_BASE_ADDRESS + PAGE_HEADER_SIZE + 16;
  /* Get page1 start address */
  uint32_t Page1_Start_Address = PAGE1_BASE_ADDRESS + PAGE_HEADER_SIZE + 16;
  ...
  switch (PageStatus0)
  {
    ...
    case RECEIVE_DATA:
      if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
      {
        ...
        {
          if (( *(__IO uint16_t*)(Page0_Start_Address + (VarIdx * EEPROM_ELEMENT_SIZE) + 16)) == VirtAddVarTab[VarIdx])
          {
            x = VarIdx;
          }
          if (VarIdx != x)
    ...
    case VALID_PAGE:
      ...
      else /* Page0 valid, Page1 receive */
      {
        ...
        for (VarIdx = 0; VarIdx < NB_OF_VAR; VarIdx++)
        {
          if ((*(__IO uint16_t*)(Page1_Start_Address + (VarIdx * EEPROM_ELEMENT_SIZE) + 16)) == VirtAddVarTab[VarIdx])
          {
            x = VarIdx;
          }
          if (VarIdx != x)
