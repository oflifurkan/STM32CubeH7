...
In order to make the program work, you must do the following:
  ...
  * In order to load the application to the internal Flash memory:
    ...
    2. Open your preferred toolchain
       - Open the Project
       - Use the "XIP_QSPI_InternalSRAM" project
       - Rebuild all files
  ...
  * In order to load the application to the external memory:
    ...
    1. Select required configuration in memory.h in Templates\ExtMem_Boot\Inc template.
       ...
       - DATA_AREA set to USE_EXTERNAL_SDRAM
       - CODE_AREA set to USE_QSPI

    2. Open your preferred toolchain
       - Open the Project
       - Use the "XIP_QSPI_ExternalSDRAM" project
       - Rebuild all files
       - Run & debug the program

  * For a BootROM configuration (BootROM external SDRAM):

    @Note : Loading the application code is only with STM32CubeProgrammer tool

    1. Select required configuration in memory.h in Templates\ExtMem_Boot\Inc template.
       The configuration is:
       - DATA_AREA set to USE_INTERNAL_SRAM
       - CODE_AREA set to USE_EXTERNAL_SDRAM
       - BINARY_AREA set to USE_SPI_NOR or USE_MMC
    2. Open your preferred toolchain
       - Open the Project
       - Use the "BootROM_ExternalSDRAM_InternalSRAM" project
       - Rebuild all files

    * In order to load the application binary:

      - If BINARY_AREA is USE_SPI_NOR:
     ...
     - If BINARY_AREA is USE_MMC:
       - Open the STM32CubeProgrammer tool
       - Select the eMMC loader "MTFC4GACAJCN_STM32H750B-DISCO" in case of XIP from QSPI
       - From Erasing & Programming menu, browse and open the output binary file relative to your application
       - Load the file into the external QSPI flash using "Start Programming" at the address APPLICATION_ADDRESS (0xA0000000)
       - Perform a reset
